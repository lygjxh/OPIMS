...
## 6A. 附件数据接口规格（2026-07-27 新增）

本章定义系统需要**解析内容**的三张 Excel 表的字段规格，是编写解析代码的直接依据。
其余附件（A、C、F）系统只登记「是否提交」，不解析内容。

### 6A.0 通用解析约定

| 约定 | 说明 |
|---|---|
| **按标签定位，不按行号** | 键值区一律「A 列写标签、B 列写值」。解析时遍历 A 列找到标签文本再取同行 B 列，**不得写死行号**——项目部插入行不应导致解析失败 |
| **标签匹配去空白** | 比对前对标签做 `TrimSpace`，并容忍全角/半角括号差异 |
| **表格区靠表头行锚定** | 多行表格先找到表头行（匹配首列表头文字），数据从下一行开始，**读到整行为空即停止** |
| **数值清洗** | 去除千分位逗号、货币符号、百分号与前后空白后再转数字；转换失败记为「该字段无效」并列入解析告警，不中断整表解析 |
| **日期格式** | 统一 `YYYY-MM-DD`。Excel 日期单元格与文本日期都要能读；无法解析记为空并告警 |
| **项目简称是关联键** | 表内项目简称必须与 `projects.short_name` 完全一致；不一致时不猜测、不模糊匹配，直接记为「无法关联」并告警 |
| **单文件单项目单期** | 每个文件只承载一个项目一个周期的数据，避免跨项目合并表带来的歧义 |

### 6A.1 附件 B-1：月度进度数据表

- 文件名：`{项目简称}-MPD-YYYYMM.xlsx`
- 存放：`01.Project Files/{项目简称}/06.Report/Monthly Report/`
- 报送频率：月

#### Sheet「进度数据」——键值区（A 列标签，B 列值）

| 标签（精确文本） | 类型 | 必填 | 校验与说明 |
|---|---|:-:|---|
| 项目简称 | 文本 | 是 | 须等于 `projects.short_name` |
| 报告周期 | 文本 | 是 | `YYYY-MM` |
| 数据日期 | 日期 | 是 | `YYYY-MM-DD`，须落在报告周期当月或次月初 |
| 填报人 | 文本 | 否 | |
| 关键路径滞后天数 | 整数 | 是 | 可为负（提前）；非整数记为无效 |
| 付款里程碑预计滞后 | 枚举 | 是 | `是` / `否` |
| 本期已产生 LDs | 枚举 | 是 | `是` / `否` |
| 项目自评风险灯 | 枚举 | 是 | `绿` / `黄` / `橙` / `红` |
| 币种 | 文本 | 是 | 如 `USD` / `CNY` |
| 本期计划产值 | 数字 | 是 | ≥0 |
| 本期实际产值 | 数字 | 是 | ≥0 |
| 累计计划产值 | 数字 | 是 | ＞0（作为 SPI 分母） |
| 累计实际产值 | 数字 | 是 | ≥0 |

> 键值区还包含「本期完成率／累计完成率／SPI／系统风险灯」四行，值为「系统计算」字样，
> **解析时忽略这四行**——它们是给填报人看的占位提示。

#### Sheet「里程碑」——表格区

表头固定文字：`里程碑名称 | 是否付款节点 | 合同约定日期 | 当前预计/实际完成日期 | 状态`
数据自表头下一行起，读到整行为空为止。
...
| 里程碑名称 | 文本 | 非空 |
| 是否付款节点 | 枚举 | `是` / `否` |
| 合同约定日期 | 日期 | `YYYY-MM-DD` |
| 当前预计/实际完成日期 | 日期 | 可为空（未开始） |
| 状态 | 枚举 | `未开始` / `进行中` / `已完成` / `预计滞后` |
...
本期完成率 = 本期实际产值 / 本期计划产值
累计完成率 = 累计实际产值 / 累计计划产值
SPI       = 累计实际产值 / 累计计划产值        // 分母为 0 时记为无效，不算灯
里程碑达成率 = 状态为「已完成」的里程碑数 / 合同约定日期已到期的里程碑数
```

**风险灯判定（严格照搬细则 4.1，取最严重项）**：

| 灯 | 条件 |
|:-:|---|
| 绿 | 滞后 ≤3 天 **且** SPI ≥0.95 |
| 黄 | 滞后 3–7 天 **或** SPI 0.90–0.95 |
| 橙 | 滞后 7–14 天 **或** SPI 0.85–0.90 **或** 付款里程碑预计滞后=是 |
| 红 | 滞后 >14 天 **或** SPI <0.85 **或** 本期已产生 LDs=是 |

系统须同时输出细则 4.1 对应的**响应要求**文本（如橙灯→「出正式纠偏计划，报中心备案」），
避免「看到灯不知道该做什么」。

**自评与系统判定不一致时**，标记为「待核实」并在看板高亮——这是低成本的交叉校验，
不一致往往意味着数据填报有误或存在系统未覆盖的特殊情况。

### 6A.2 附件 E：合同时效日历

- 文件名：`{项目简称}-TBC-YYYYMM.xlsx`
- 存放：`01.Project Files/{项目简称}/05.Schedule/Time Bar/`

#### Sheet「时效日历」——表格区

表头文字：`序号 | 合同条款号 | 时效代码 | 触发事件描述 | 触发日期 | 时限天数 | 到期日 | 剩余天数 | 预警灯 | 实际完成日 | 状态 | 责任人 | 备注`

| 列 | 类型 | 必填 | 说明 |
|---|---|:-:|---|
| 合同条款号 | 文本 | 是 | 如 `GC 20.1` |
| 时效代码 | 枚举 | 是 | `T1`–`T6`、`T9`，见 6A.4 |
| 触发事件描述 | 文本 | 是 | |
| **触发日期** | 日期 | 是 | 时效起算日，最关键字段 |
| **时限天数** | 整数 | 是 | **日历日**，如 10、21、28 |
| 到期日 / 剩余天数 / 预警灯 | — | — | **解析时跳过，由系统计算** |
| 实际完成日 | 日期 | 否 | 空表示尚未完成 |
| 状态 | 枚举 | 否 | `未启动` / `进行中` / `已完成` / `已逾期` |
| 责任人 | 文本 | 否 | |

#### 系统计算规则

```
到期日   = 触发日期 + 时限天数            // 日历日，不扣除周末与节假日
剩余天数 = 到期日 - 当前日期              // 每次查询实时计算，不落库
预警灯   = 已完成           → 灰（停止倒计时）
           剩余 <7 或已逾期 → 红
           剩余 7–14        → 黄
           剩余 >14         → 绿
...
⚠️ **必须按日历日计算**。细则 V1.1 第 5.1 节已统一为日历日，理由是各国假日不同、
按工作日计算易错且难跨项目核对；日历日得出的到期日更早，属安全方向。
**不要引入工作日/节假日表**，那会让规则复杂且各国口径不一。

⚠️ **剩余天数与预警灯不得落库**。它们随日期变化，落库即过期。每次查询实时算。

### 6A.3 附件 D：工期索赔跟踪台账

- 文件名：`{项目简称}-EOT-YYYYMM.xlsx`
- 存放：`01.Project Files/{项目简称}/05.Schedule/EOT/`
...
| 序号 | 整数 | 项目内唯一 |
| 索赔事项 | 文本 | |
| 类别代码 | 枚举 | `C1`–`C9`，见附件 D 定义 |
| 合同依据条款 | 文本 | |
| 发现/发生日期 | 日期 | |
| CN 发出日 | 日期 | 空=尚未发出 |
| CN 时限到期日 / PCO 应交日 | — | **解析时跳过，由系统按下式计算** |
| PCO 实交日 | 日期 | |
| 累计影响天数 | 整数 | |
| 累计索赔金额 | 数字 | |
| 当前状态 | 枚举 | `待发CN`/`CN已发待PCO`/`PCO已交待批`/`业主已批准`/`业主已驳回`/`滚动更新中`/`已关闭` |

```
CN 时限到期日 = 发现/发生日期 + 10 日历日
PCO 应交日    = CN 发出日 + 21 日历日
逾期判定      = 到期日已过且对应实际日期为空
```

> **与附件 E 的关系**：附件 D 跟踪「一笔索赔的全过程」，附件 E 跟踪「一条时效的倒计时」。
> 一个索赔事项通常对应 E 表中的多条记录（CN 一条 T1、PCO 一条 T2）。
> 系统应能由 D 表自动派生 E 表条目，减少重复录入——**但一期先各自独立解析，
> 派生逻辑放二期**，避免一次引入过多耦合。
...
| 是否类 | `是` / `否` |
| 风险灯 | `绿` / `黄` / `橙` / `红` |
| 里程碑状态 | `未开始` / `进行中` / `已完成` / `预计滞后` |
| 时效状态 | `未启动` / `进行中` / `已完成` / `已逾期` |
| 时效代码 | `T1` 变更/延误通知(10日) · `T2` PCO提交(21日) · `T3` 不可抗力通知(7–14日) · `T4` 索赔详细报告(28–56日) · `T5` 滚动PCO更新(21–28日) · `T6` 业主答复期届满 · `T9` 其他 |
| 索赔类别 | `C1` 业主的行为 · `C2` 业主代表的行为 · `C3` 业主设计变更 · `C4` 合同文件缺陷 · `C5` 施工条件变化 · `C6` 所在国政策法规变更 · `C7` 不可抗力 · `C8` 不可预见因素 · `C9` 业主指定分包商违约 |

> 枚举值应集中定义在 `backend/data/` 下，不要散落在解析代码里——细则修订时只改一处。
...
|---|---|
| 文件不存在 | 记为「缺交」，不报错 |
| 文件存在但打不开（损坏/格式不符） | 记为「已交但解析失败」，列入告警，不中断其它项目扫描 |
| 必填字段缺失或无效 | 该字段记为空 + 告警，其余字段照常入库 |
| 项目简称无法关联 | 整表记为「无法关联」并告警，不猜测归属 |
| 枚举值不在取值范围 | 记为空 + 告警，不做同义词猜测（如「延期」不自动当作「预计滞后」） |

**告警须能在界面上看到**（建议与「待人工识别」清单合并展示），否则解析失败会被静默忽略，
管理员以为数据正常，实则看板是错的。
