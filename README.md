...
> **接手改代码前请先读 [CHANGELOG.md](CHANGELOG.md)**（按时间倒序，最新在最上面）。
> 那里记录了每次改动的原因和踩过的坑——包括几处「看起来像 bug 其实是有意为之」
> 和「报错信息具有误导性」的情况，不看容易把修好的东西又改回去。
> 本文件末尾的「注意事项（易踩的坑）」一节也建议先扫一遍。
...
│       ├── data/regions.ts     # 国别→地区（大区）映射（合同额分布用）
│       ├── views/              # 页面组件
│       ├── i18n/               # 中英双语
│       └── router/             # 路由（hash 模式）
├── docs/                       # 功能介绍与操作说明
...
| 板块 | 状态 |
|---|---|
| 首页看板 | ✅ 已实现（KPI 卡片、全球分布地图、合同额分布饼图、快捷入口） |
...
- 环形饼图（纯 SVG 手绘，**无第三方图表库依赖**，便于离线与后续鸿蒙移植）
- 维度可切换：按地区（默认，口径见 `frontend/src/data/regions.ts`）/ 按国别
- 状态筛选：全部（单选）+ 在建/未开工/完工（可多选，取并集）
- 按地区时点击扇区或图例可下钻查看国别明细，下钻沿用当前状态筛选
- 分组超过 7 个时尾部合并为「其他 N 项」；**不要通过生成新色相来容纳更多分组**——新色相在色盲下无法与既有色区分
- 图例必须保留名称/金额/占比文字：分类色中有 3 个对比度低于 3:1，文字是必需的兜底

新增国别时需同时维护三处：`backend/services/country.go`（识别关键词）、`frontend/src/data/countryCoords.ts`（坐标）、`frontend/src/data/regions.ts`（所属地区）。
